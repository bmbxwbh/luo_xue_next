...
目前项目最大的卡点在于**外部音源插件的数据传递**。我们已经把洛雪原版的预加载脚本、事件通信机制、HTTP 代理、加密工具等都移植到了 Flutter 端，音源插件也能正常加载和初始化，但实际请求播放链接时，**签名（sign）的计算结果和原版不一致**，导致无法获取到播放地址。
...
**可能的原因：**
- flutter_js 和原版 QuickJS 在某些 JS 行为上存在细微差异
- 插件脚本内部的 SHA-256 实现在 flutter_js 环境下有兼容性问题
- 传入插件的数据在某些边界情况下和原版不完全一致
