...
> **开发者正在全力冲刺高考 📚，暂时无法投入大量时间维护项目。高考结束后会回来继续更新，感谢大家的理解与支持！🙏**
>
> 在此期间，如果有问题欢迎提 Issue，考完回来会统一处理。
...
目前项目在**洛雪脚本模式**下存在一个卡点。我们已经把洛雪原版的预加载脚本、事件通信机制、HTTP 代理、加密工具等都移植到了 Flutter 端，洛雪格式的音源插件也能正常加载和初始化，但使用 **洛雪独家音源.js** 实际请求播放链接时，**签名（sign）的计算结果和原版 QuickJS 环境下不一致**，导致无法获取到播放地址。

> 💡 **注意：** MusicFree 插件模式已完整实现并测试通过，支持搜索、播放、歌词、歌单等全部功能。本求助仅针对洛雪脚本模式下的 sign 计算问题。
...
- ❓ 但在 flutter_js 环境下，洛雪独家音源.js 计算出的 sign 值与原版 QuickJS 环境下不同
