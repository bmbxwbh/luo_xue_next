...
class _PlayDetailScreenState extends State<PlayDetailScreen>
  ...
  bool get _isGlass => context.mounted && context.read<SettingStore>().appStyle == 'liquid_glass';
  ...
  Widget _glassProgress(BuildContext ctx, double progressVal, double maxMsVal, ColorScheme colorScheme) {
    return GestureDetector(
      onHorizontalDragUpdate: (details) {
        final box = ctx.findRenderObject() as RenderBox;
        final ratio = (details.localPosition.dx / box.size.width).clamp(0.0, 1.0);
        if (maxMsVal > 0) {
          globalPlayer.seekTo(Duration(milliseconds: (maxMsVal * ratio * 1000).round()));
        }
      },
      child: Container(
        height: 20,
  ...
  Widget _buildMD3Progress(dynamic progress, ColorScheme colorScheme) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 4),
      child: Row(
        children: [
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          // 进度条
          isGlass
              ? _glassProgress(context, progress.progress, progress.maxPlayTime.toDouble(), colorScheme)
              : _buildMD3Progress(progress, colorScheme),
          const SizedBox(height: 4),
          // 播放控制按钮
          isGlass
              ? _buildGlassControls(colorScheme, progress)
              : _buildMD3Controls(colorScheme, progress),
  ...
  Widget _buildGlassControls(ColorScheme colorScheme, dynamic progress) {
    return Column(
      children: [
        // 主控制按钮
        Row(
          ...
          children: [
            _buildGlassBtn(icon: _playModeIcon(globalSettingStore.playMode), size: 24, onTap: () => globalPlayer.togglePlayMode()),
            _buildGlassBtn(icon: Icons.skip_previous_rounded, size: 32, onTap: () => globalPlayer.playPrevious()),
            // 播放/暂停 — 玻璃主按钮
            GestureDetector(
            ...
            _buildGlassBtn(icon: Icons.skip_next_rounded, size: 32, onTap: () => globalPlayer.playNext()),
            _buildGlassBtn(icon: Icons.queue_music_rounded, size: 24, onTap: () => _showPlaylist()),
          ],
        ),
        const SizedBox(height: 8),
        // 底部操作栏
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            _buildGlassBtn(icon: Icons.favorite_border_rounded, size: 24, onTap: _addToFavorite),
            _buildGlassBtn(icon: Icons.speed, size: 20, onTap: () => _showSpeedPicker(context)),
            _buildGlassBtn(icon: Icons.download_rounded, size: 24, onTap: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('下载功能开发中'), behavior: SnackBarBehavior.floating, duration: Duration(seconds: 1)),
              );
            }),
          ],
        ),
  ...
  Widget _buildGlassBtn({required IconData icon, required double size, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
  ...
  Widget _buildMD3Controls(ColorScheme colorScheme, dynamic progress) {
    return Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            _buildControlBtn(icon: _playModeIcon(globalSettingStore.playMode), size: 24, onTap: () => globalPlayer.togglePlayMode()),
            _buildControlBtn(icon: Icons.skip_previous_rounded, size: 32, onTap: () => globalPlayer.playPrevious()),
            GestureDetector(
              onTap: () => globalPlayer.togglePlay(),
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 200),
            ...
            _buildControlBtn(icon: Icons.skip_next_rounded, size: 32, onTap: () => globalPlayer.playNext()),
            _buildControlBtn(icon: Icons.queue_music_rounded, size: 24, onTap: () => _showPlaylist()),
          ],
        ),
        const SizedBox(height: 8),
        Row(
          ...
          children: [
            IconButton(
              icon: Icon(_playModeIcon(globalSettingStore.playMode), size: 24, color: colorScheme.onSurfaceVariant),
              onPressed: () => globalPlayer.togglePlayMode(),
            ),
            IconButton(
              icon: Icon(Icons.favorite_border_rounded, size: 24, color: colorScheme.onSurfaceVariant),
              onPressed: _addToFavorite,
            ),
            _buildSpeedBtn(context),
            IconButton(
              icon: Icon(Icons.download_rounded, size: 24, color: colorScheme.onSurfaceVariant),
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('下载功能开发中'), behavior: SnackBarBehavior.floating, duration: Duration(seconds: 1)),
                );
  ...
  void _showSpeedPicker(BuildContext context) {
    ...
    final glass = _isGlass;
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (_) => glass
          ? _GlassSpeedSheet(store: store, context: context)
          : _buildSpeedSheet(context, store),
    );
  }

  Widget _buildSpeedSheet(BuildContext context, SettingStore store) {
...
/// 玻璃倍速选择器
class _GlassSpeedSheet extends StatelessWidget {
  final SettingStore store;
  final BuildContext context;

  const _GlassSpeedSheet({required this.store, required this.context});
