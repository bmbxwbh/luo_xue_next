...
class _TabSongListState extends State<TabSongList> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        // 分类标签 — 上滑隐藏，下滑显示
        AnimatedContainer(
          height: _categoriesVisible ? 40.0 : 0.0,
  ...
  bool _onContentScroll(ScrollNotification n) {
    if (n is ScrollUpdateNotification) {
      ...
      if (delta > 4 && _categoriesVisible) {
        setState(() => _categoriesVisible = false);
      } else if (delta < -4 && !_categoriesVisible) {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: _buildChip(cat.name, selected),
          );
        },
      ),
    );
  ...
  Widget _buildChip(String label, bool selected) {
    ...
    return GestureDetector(
      onTap: () => _onCategoryChanged(label, _categories.firstWhere((c) => c.name == label).tagId),
